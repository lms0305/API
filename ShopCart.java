...
public class ShopCart {
     ...
     * @return
     */
    public List<GoodsInfo> removeShopCart(GoodsInfo goodsInfo1,Integer uid){
        Gson gson = new Gson();
        ...
        if (r==null){
            ...
            String isHaveGoods = shopMap.get(""+goodsInfo1.getGid());
            //购物车中有此商品
            if (isHaveGoods!=null){
                //获取购物车中商品信息
                String shopGoodsInfoJson = shopMap.remove("" + goodsInfo1.getGid());
                String shopMapJson = gson.toJson(shopMap);
                redisUtils.hset("shopCart",""+uid,shopMapJson);
                //遍历返回
                for(String s:shopMap.keySet()){
                    String s1 = shopMap.get(s);
     ...
     * @return
     */
    public List<GoodsInfo> addOrUpdateShopCart(GoodsInfo goodsInfo1,Integer uid){
        Gson gson = new Gson();
        ...
        if (r==null){
            ...
            //存入redis
            redisUtils.hset("shopCart", ""+uid,goodsInfoMapJson);
        }else {
            //用户已经有了购物车
